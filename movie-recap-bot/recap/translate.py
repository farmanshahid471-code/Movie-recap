...
def check_alignment(en_lines: list[str], zh_lines: list[str]) -> tuple[bool, str]:
    """Sanity-check that a ZH translation stayed line-aligned with the EN.

    A translation that merges or splits lines silently drifts the ZH visuals
    (each ZH sentence is paired with the EN sentence's film window by index),
    so a mismatch should be surfaced instead of producing a subtly wrong video.
    """
    if len(zh_lines) == len(en_lines):
        return True, ""
    return (
        False,
        f"ZH translation has {len(zh_lines)} lines but the EN source has "
